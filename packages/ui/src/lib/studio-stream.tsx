...
export function createPerKeyThrottle(
  ...
  return {
    trigger(key) {
      ...
      pending.set(key, setTimeout(() => { pending.delete(key); fn(key) }, ms))
    },
    dispose() {
...
function StreamBridge({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    es.addEventListener("snapshot", (e) => {
      const { sessions } = JSON.parse((e as MessageEvent).data) as { sessions: SessionSummary[] }
      queryClient.setQueryData(["sessions"], sessions)
    })
    es.addEventListener("update", (e) => {
      const u = JSON.parse((e as MessageEvent).data) as RegistryUpdate
      if (u.kind === "session") {
        queryClient.setQueryData<SessionSummary[]>(["sessions"], (prev) => applySessionUpdate(prev, u.session))
      } else {
        throttle.trigger(u.sessionId)
      }
    })
    return () => { throttle.dispose(); es.close() }
  }, [])
