...
const fixture = fileURLToPath(
  new URL(
    "../../studio/test/fixtures/mock-eval-envelopes.ndjson",
    import.meta.url
  )
)
// The eve pin's single source of truth is packages/studio/package.json dependencies.eve.
const evePin = JSON.parse(
  readFileSync(
    fileURLToPath(new URL("../../studio/package.json", import.meta.url)),
    "utf8"
  )
).dependencies.eve
...
console.log(
  `dev-collector: ${registry.getSessions().length} fixture session(s) on ${server.url}`
)
console.log(
  "dev-collector: now run `pnpm dev` in packages/ui and open http://127.0.0.1:43120"
)
